...
const DashboardCard = ({ icon: Icon, title, description, link, gradient }) => (
  ...
  >
    {/* Glow effect */}
    <div
      className={`absolute inset-0 rounded-2xl ${gradient} opacity-20 group-hover:opacity-40 blur-xl transition-all duration-500`}
    />
    <Link
      to={link}
      className="block p-8 bg-gray-800/70 backdrop-blur-xl border border-white/10 rounded-2xl shadow-lg relative z-10 h-full transform transition-transform duration-300"
    >
      ...
      <div className="mt-6">
        <span className="inline-flex items-center text-blue-400 hover:text-blue-300 transition-colors font-medium">
          Get Started
          <svg
...
const ProfileCard = ({ user, onLogout }) => (
  <motion.div
    variants={itemVariants}
    className="bg-gray-800/70 backdrop-blur-xl border border-white/10 rounded-2xl p-6 relative overflow-hidden"
  >
  ...
  const dashboardCards = [
    {
      ...
      description:
        'Craft and customize your professional resume with AI-powered suggestions.',
      link: '/resume',
    ...
    },
    {
      ...
      title: 'Cover Letter',
      description:
        'Generate personalized cover letters tailored for every job application.',
      link: '/cover-letter',
